...
[下载最新版本](../../releases/latest) · [查看公开回测报告](./BACKTEST.md) · [申请试用](https://t.me/polymarket_b)
...
当前公开报告使用生产代码逻辑，按历史时间顺序逐根重放。截止北京时间 2026-09-01 11:35，统计摘要如下：

| 指标 | 结果 |
| --- | ---: |
| 统计区间 | 2021-09-01 至 2026-09-01 |
| 总信号数 | 18,633 |
| 实际执行订单 | 18,285 |
| 策略 STOP | 348（1.87%） |
| 胜 / 负 | 11,656 / 6,629 |
...
报告使用固定 `+4/-5` 标准化计分，理论盈亏平衡胜率约为 55.56%。标准化得分和回撤用于比较方向质量与历史路径，不是百分比或 pUSD 盈亏。2022—2025 四个完整自然年的执行胜率介于 61.78% 至 65.64%。近期滚动结果在回测报告中更新。

完整统计口径、年度与近期结果、回撤及数据局限见[公开回测报告](./BACKTEST.md)。历史行情曾用于策略研究，因此五年结果反映的是当前规则在历史时序下的表现和风险路径，不代表一段完全独立的未接触样本。
...
| 信号策略包 | 当前版本最新策略包 | 当前建议 `V4 Final + L2 Opt`，升级后以新版默认值为准 |
...
> `V4 Final + L2 Opt` 是界面中的策略包名称；V16 是该策略包当前内部规则的迭代版本。两者描述的层级不同，不是两套相互冲突的策略。
...
影子“停止”“L2 停止”“V4·RS1停止”或“停止·有候选”可以是已经准备完成后的有效策略状态，不需要等待所有影子变成“启动”。首次准备完成后，后台通常只处理新增已收盘 K 线和断点补齐，不会在每个周期重复完整历史重建。
