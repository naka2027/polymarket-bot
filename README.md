...
> **功能升级：现已支持 OKX（欧易）BTCUSDT 永续合约量化交易。**
>
> 新增独立的 OKX 合约自动化链路，支持模拟盘与实盘、逐仓模式、动态杠杆、仓位与保护单维护、异常恢复和订单记录，并可与 Polymarket 自动化独立运行。先阅读 [OKX BTC 合约策略思路（公开版）](./STRATEGY_OKX.md)，完整回放方法、结果和限制见 [OKX BTC 合约公开回测报告](./BACKTEST_OKX.md)。
...
- [OKX BTC 合约策略思路（公开版）](./STRATEGY_OKX.md)
- [OKX BTC 合约公开回测报告](./BACKTEST_OKX.md)
